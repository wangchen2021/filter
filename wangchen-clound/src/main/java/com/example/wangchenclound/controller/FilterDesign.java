...
//        获取电阻
    ...
    @GetMapping("/filterDesign/test")
    public JSONObject Test(HttpServletRequest request){
        JSONObject jsonObject=new JSONObject();
        double Zoo=Double.parseDouble(request.getParameter("Zoo"));
        double Zoe=Double.parseDouble(request.getParameter("Zoe"));
        double Kr=Double.parseDouble(request.getParameter("Kr"));
        double WHoe=microStripParam.getWHoe(0.5*Zoe,Kr);
        double SH1= microStripParam.SH1(Zoo,Zoe);
        double WH1= microStripParam.WH1(SH1,WHoe);
        double ZoA= microStripParam.ZoA(SH1,WH1);
        double Zoo2= microStripParam.Zoo2(Zoo,ZoA);
        double WHso=microStripParam.getWHoe(0.5*Zoo2,Kr);
        jsonObject.put("(W/H)oe",WHoe);
        jsonObject.put("(S/H)1",SH1);
        jsonObject.put("(W/H)1",WH1);
        jsonObject.put("ZoA",ZoA);
        jsonObject.put("Zoo2",Zoo2);
        jsonObject.put("(W/H)so",WHso);
        return jsonObject;
    }
}